...
## TASK-DB-006: Database Schema Design and Documentation
...
**Dependencies:** TASK-DB-006, TASK-ARCH-001
...
## TASK-DOC-006: Authentication and Authorization Design
...
**Estimated Time:** 6 hours
**Dependencies:** TASK-ARCH-001
**Related Tasks:** TASK-DB-005, TASK-API-005, TASK-SEC-003

### Description

Design comprehensive authentication and authorization system for the LuxeNail application. This includes defining user roles, permissions, authentication flows, authorization rules, and security requirements. This design will serve as the foundation for database schema, API implementation, and frontend authentication integration.

### Requirements / What to Do

#### Step 1: Define User Roles and Hierarchy

- [ ] Document all user roles (already defined in types.ts):
  - **CUSTOMER**: End users who book appointments
    - Can view public pages (landing, gallery, services)
  ...
  - **EMPLOYEE**: Staff members who provide services
    - Can view their own schedule/appointments
...
- [ ] Create permission matrix document showing:
  - Resource/action permissions for each role
...
- [ ] Identify permission exceptions:
  - Employees can view own data but not others
...
- [ ] **Route Protection:**
  - Public routes: Landing, Gallery, Services, About
  - Protected routes: Dashboard, Profile, Appointments
  - Role-based route access
- [ ] **API Endpoint Protection:**
  - Public endpoints: GET /api/services, GET /api/employees
...
- [ ] **Field-Level Authorization:**
  - Sensitive fields (financial data, personal info) restricted to appropriate roles
...
- [ ] **Admin/Owner Registration:**
  - Only existing OWNER can create ADMIN accounts
...
- [ ] Create `doc/AUTHENTICATION_DESIGN.md`:
...
2. **Stakeholder Validation:**
   - Present design to business owner
...
- ✅ Complete user role hierarchy defined
...
**Related Tasks:** TASK-API-005

### Description

Implement database schema for user authentication and authorization system. This includes extending the users table with authentication fields, creating roles and permissions tables, implementing password storage, and setting up authorization data structures.

### Requirements / What to Do

#### Step 1: Extend Users Table with Authentication Fields

- [ ] Update `server/src/entities/User.entity.ts`:
  - Add `email` field (VARCHAR, unique, required, indexed)
...
- [ ] Create `server/src/entities/RefreshToken.entity.ts`:
  - `id` (UUID, primary key)
...
- [ ] Create `server/src/entities/RolePermission.entity.ts`:
  - `id` (UUID, primary key)
...
- [ ] Create `server/src/entities/PasswordHistory.entity.ts`:
  - `id` (UUID, primary key)
  - `userId` (UUID, foreign key to User, required, indexed)
...
- [ ] Create `server/src/entities/LoginAttempt.entity.ts`:
  - `id` (UUID, primary key)
...
- [ ] Create migration for extending User entity:
...
- [ ] Create migration for RefreshToken entity:
  - `server/src/migrations/XXXXXX-CreateRefreshToken.ts`
- [ ] Create migration for PasswordHistory entity:
  - `server/src/migrations/XXXXXX-CreatePasswordHistory.ts`
- [ ] Create migration for LoginAttempt entity:
  - `server/src/migrations/XXXXXX-CreateLoginAttempt.ts`
- [ ] Create migration for Permission and RolePermission (if using):
  - `server/src/migrations/XXXXXX-CreatePermissionTables.ts`
  - Seed default permissions
  - Seed role-permission mappings

#### Step 8: Seed Initial Data

- [ ] Create seed data for default OWNER account:
  ...
  - Role: OWNER
- [ ] Create seed data for permissions (if using permission system):
  - Define all permissions for each resource/action
...
- [ ] User entity extended with all authentication fields
...
3. **Data Integrity Test:**
   - Insert sample user with authentication data
...
## TASK-API-005: Authentication and Authorization API
...
**Related Tasks:** TASK-SEC-003

### Description

Implement complete authentication and authorization API endpoints and middleware for the LuxeNail application. This includes login, registration, password management, JWT token handling, role-based access control, and authorization middleware.
...
- [ ] Create/Update `server/src/services/UserService.ts`:
  - `registerCustomer(data: RegisterDto): Promise<User>` - Register new customer
...
- [ ] Create `server/src/middleware/auth.middleware.ts`:
  ...
  - `requireAuth` middleware:
    - Wrapper that ensures user is authenticated
    - Used for protected routes

#### Step 7: Create Authorization Service

- [ ] Create `server/src/services/AuthorizationService.ts`:
  - `canAccessResource(user: User, resource: string, action: string): Promise<boolean>` - Check permission
...
## TASK-SEC-003: Authentication Security Implementation
...
**Related Tasks:** TASK-DOC-006

### Description

Implement additional security measures for the authentication system, including secure token handling, session management, security headers, and security monitoring. This complements TASK-API-005 with advanced security features.

### Requirements / What to Do

#### Step 1: Implement Secure Token Storage

- [ ] Configure token storage strategy:
  - Store refresh tokens in HTTP-only cookies (recommended)
  - Or store in database (current implementation)
...
- [ ] Implement token blacklist:
  ...
  - Use for logout and password change scenarios

#### Step 2: Implement HTTPS Enforcement
...
- [ ] Validate secure context:
  - Ensure tokens only transmitted over HTTPS
  - Warn in logs if tokens used over HTTP in production
- [ ] Configure security headers:
  - Content-Security-Policy
...
- [ ] Implement concurrent session limits:
  ...
  - Or: Limit to 3 concurrent sessions
...
- [ ] Create security alerts:
  ...
  - Email notification on login from new IP/device
  - Email notification on password change
  - Email notification on sensitive actions
- [ ] Implement security dashboard (future):
...
- [ ] Sanitize authentication inputs:
  ...
  - Password: Strip whitespace (but allow spaces in middle)
...
- [ ] Or use SameSite cookie attribute:
  ...
  - Prevents CSRF for most scenarios
...
- [ ] Configure Helmet.js for security headers:
  ...
  - X-XSS-Protection
  - Referrer-Policy
- [ ] Configure per-route headers if needed
- [ ] Test headers in production

#### Step 8: Implement Password Security Enhancements
...
- [ ] Password breach checking (optional):
  ...
  - Warn users if password found in breaches
  - Recommend password change
- [ ] Password expiration policy (optional):
  - Enforce password change every 90 days for staff
  - Send reminder emails before expiration
  - Force password change on expiration

#### Step 9: Implement Two-Factor Authentication (Future, Optional)
...
#### Step 10: Create Security Documentation
...
- [ ] Security headers configured
- [ ] Password security enhancements implemented
...
1. **Security Testing:**
   - Test token security (no XSS vulnerabilities)
...
**Files to Modify:**
- `server/src/middleware/auth.middleware.ts` (add token blacklist check)
- `server/src/index.ts` (configure security headers)

**Dependencies:**
- Helmet.js (security headers)
- express-rate-limit (rate limiting)
- csrf (CSRF protection, if needed)
